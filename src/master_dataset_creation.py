...
eco_agg = (
    eco_stats.groupby(["Match Name", "Team", "Type"])["Won"]
    .sum()
...
kills_agg = (
    kills_stats.groupby(["Match Name", "Team"])
    .agg(
        total_2k=("2k", "sum"),
        total_3k=("3k", "sum"),
        total_4k=("4k", "sum"),
...
draft_agg = (
    draft_phase.groupby(["Match Name", "Team", "Action"])
    .size()
    .unstack(fill_value=0)
    .reset_index()
)
...
for team_col, prefix in [("Team A", "ta"), ("Team B", "tb")]:
    ...
    temp.columns = ["Match Name", "Team"] + [f"{prefix}_{c}" for c in temp.columns[2:]]
    master = master.merge(
        temp.rename(columns={"Team": team_col}),
        on=["Match Name", team_col], how="left"
    )

    temp = eco_agg.copy()
    temp.columns = ["Match Name", "Team"] + [f"{prefix}_eco_{c}" for c in temp.columns[2:]]
    master = master.merge(
        temp.rename(columns={"Team": team_col}),
        on=["Match Name", team_col], how="left"
    )
    ...
    temp = draft_agg.copy()
    temp.columns = ["Match Name", "Team"] + [f"{prefix}_draft_{c}" for c in temp.columns[2:]]
    master = master.merge(
        temp.rename(columns={"Team": team_col}),
        on=["Match Name", team_col], how="left"
    )
